...
void main(void) {
	...
	vec3 position = matrix * a_position;
	gl_Position = vec4(position, 1);

	// In my engine this is:
	// uvMatrix.setIdentity();
	// uvMatrix = uvMatrix.scale(1, -1); // Flip UVs to opengl format
	// uvMatrix = uvMatrix.translate(0, -1); // Flip UVs to opengl format
	...
	v_texCoord = vec4((flipMatrix * uvMatrix * a_position).xy, 0, 1).xy;
...
void main() {
	...
	if ((flags & _F_RECT_AA) != 0) {
		highp vec2 uvPixel = fwidth(v_texCoord);
