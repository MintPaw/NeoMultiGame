...
void DrawMarker(const ImVec2& pmin, const ImVec2& pmax, const ImU32& color, DrawMarkerMode mode)
{
	...
	auto w = static_cast<int32_t>(pmax.x - pmin.x);
	auto h = static_cast<int32_t>(pmax.y - pmin.y);
	...
	MarkerDirection markerDir)
{
	...
	for (int i = 0; i < markerCount; i++)
	{
		...
		{
			DrawMarker(
				{originPos.x + x - markerWidth/2, originPos.y + markerHeight},
				{originPos.x + x + markerWidth/2, originPos.y + 0},
