 ...
 *                                                                             *
 *******************************************************************************
 ********** Chenglite Copyleft (No Copyright) 2018 to Maksim Korzh *************
 *******************************************************************************/
...
#define Char2Piece(ch) CHEN_pieceChars[ch - '0']
...
#define CHEN_PrintBoard(board) PrintPosition(board); PrintStats(board)
...
static inline int MakeMove(CHESSBOARD *board, int move, int capFlag) {
	...
	if(!capFlag) {
		...
		// update kingSq
		if(GetSq(GetMoveTarget(move)) == wK || GetSq(GetMoveTarget(move)) == bK) kingSq(side) = GetMoveTarget(move);	
		...
		if (GetMoveCaptureFlag(move)) MakeMove(board, move, all);
		else return 0;
	}
...
void CHEN_InitSearch(SEARCH *info)
{
	info->nodes = 0;
	info->fhf = 0;
	info->fh = 0;
...
static inline void CHEN_SearchPosition(CHESSBOARD *board, SEARCH *info, int depth)
{
	// iterative deepening
...
void CHEN_ParseFen(CHESSBOARD *board, char *fen)
{
	ResetBoard(board);
...
void UciLoop(CHESSBOARD *board, SEARCH *info)
{
	...
	while(1)
	{
		...
		else if (!strncmp(line, "go depth", 8))
		{
			...
			CHEN_SearchPosition(board, info, depth);
		}
			...
			CHEN_SearchPosition(board, info, 5);
	}
}
...
int main()
{
	CHESSBOARD board[1];
	SEARCH info[1];
	CHEN_InitSearch(info);
	
	//CHEN_ParseFen(board, "r1b1k2r/ppppnppp/2n2q2/2b5/3NP3/2P1B3/PP3PPP/RN1QKB1R w KQkq - 0 1");
	//CHEN_PrintBoard(board);
	
	
	//CHEN_SearchPosition(board, info, 6);
