...
void updateEmitter(Emitter *emitter, float elapsed) {
  ...
  if (emitter->reloadInfo) {
	...
	for (int i = 0; i < emitter->particlesNum; i++) {
		...
		Vec2 velo = v2();
		if (keyPressed('A')) {
			velo = lerp(particle->veloStart, particle->veloEnd, perc);
			if (!info->animateSpeed) velo = particle->veloStart;
			applyFn(particle, &info->veloXFn, &velo.x);
		} else {
			float speed = sampleGradientFloat(info->speedGradient, perc) * particle->speedMulti * 1000;
			velo = particle->direction * speed;
		}

		float angularSpeed = 0;
		if (keyPressed('A')) {
			angularSpeed = lerp(particle->angularSpeedStart, particle->angularSpeedEnd, perc);
			if (!info->animateAngularSpeed) angularSpeed = particle->angularSpeedStart;
		} else {
			angularSpeed = sampleGradientFloat(info->angularSpeedGradient, perc) * particle->angularSpeedMulti * 10;
		}
		...
		if (keyPressed('A')) {
			...
			accel.x = sampleGradientFloat(info->accelXGradient, perc) * particle->accelXMulti * 100;
			accel.y = sampleGradientFloat(info->accelYGradient, perc) * particle->accelYMulti * 100;
		}
		...
		if (keyPressed('A')) {
			...
			particle->scale = sampleGradientFloat(info->scaleGradient, perc) * particle->scaleMulti;
		}
...
void guiInputEmitterInfo(EmitterInfo *info) {
  ...
  if (info->explode) {
	...
	guiInputGradient("Angular speed", &info->angularSpeedGradient);
...
void readEmitterInfo(DataStream *stream, EmitterInfo *info) {
  ...
  if (version >= 9) info->defaultExplodeCount = readU32(stream);
	...
	if (version >= 12) {
		info->accelXGradient = readGradient(stream);
		info->accelYGradient = readGradient(stream);
	}
