// Checkpoint
// Key door
// Dynamite
// Speed gate
struct Globals {
};

#define _FA_IS_STARTING_DOOR (1 >> 0)

enum ActorType {
	ACTOR_NONE=0,
	ACTOR_SPAWNER=1,
	ACTOR_PLAYER=2,
	ACTOR_WALL=3,
	ACTOR_DOOR=4,
	ACTOR_SPEED_GATE=5,
	ACTOR_TYPES_MAX,
};
char *actorTypeStrings[] = {
	"None",
	"Spawner",
	"Player",
	"Wall",
	"Door",
	"Speed gate",
};

struct Actor {
	ActorType type;
#define ACTOR_NAME_MAX_LEN 128
	char name[ACTOR_NAME_MAX_LEN];
	int id;
	Vec2 position;
	Vec2 size;

	ActorType typeToSpawn;

	u32 flags;

	int doorConnectionId;

	float speedGateSpeed;

	/// Unserialized
	bool isOnGround;
	Vec2 velo;

	float shellSpeed;

	bool markedForDeletion;
};

struct Map {
#define MAP_NAME_MAX_LEN 128
	char name[MAP_NAME_MAX_LEN];

#define ACTORS_MAX 512
	Actor actors[ACTORS_MAX];
	int actorsNum;
	int nextActorId;

	/// Unserialized
};

enum GameState {
	GAME_NONE,
	GAME_PLAY,
};
struct Game {
	Font *defaultFont;
	RenderTexture *gameTexture;
	RenderTexture *debugTexture;

	Globals globals;
	bool inEditor;
	float timeScale;
	float time;
	Vec2 size;
	float sizeScale;
	Vec2 mouse;

	Vec2 screenOverlayOffset;
	Vec2 screenOverlaySize;

	GameState state;
	GameState prevState;
	GameState nextState;
	float stateTransition_t;
	float stateTime;

#define MAPS_MAX 128
	Map maps[MAPS_MAX];
	int currentMap;
	int prevMap;
	int nextMap;
	float mapTransition_t;

	int destDoorId;

	Vec2 cameraTarget;
	// Vec2 visualCameraTarget;

	/// Editor/debug
	int selectedActor;
	bool draggingActor;
	Vec2 draggingActorOffset;
	int debugChangeMap;

	bool debugShowFrameTimes;
};
Game *game = NULL;

void runGame();
void updateGame();

Actor *createActor(Map *map, ActorType type);
void deleteActor(Map *map, Actor *actor);
Actor *getActor(Map *map, int id);
Actor *getActor(Map *map, char *name);
Actor *getActorOfType(Map *map, ActorType type);
Actor *getActorWithDoorConnectionId(Map *map, int doorConnectionId);
bool doesActorContain(Actor *actor, Vec2 point);
bool overlaps(Actor *actorA, Actor *actorB);
Rect getRectFromPosition(Actor *actor, Vec2 position) { return makeRect(position - actor->size/2, actor->size); }
Rect getRect(Actor *actor) { return getRectFromPosition(actor, actor->position); }

void saveMap(Map *map, int mapFileIndex);
void loadMap(Map *map, int mapFileIndex);
/// FUNCTIONS ^

void runGame() {
#if defined(_WIN32)
#if !defined(FALLOW_INTERNAL) // This needs to be a macro
	snprintf(projectAssetDir, PATH_MAX_LEN, "%s", exeDir);
#else
#endif
#endif

	initFileOperations();

	initPlatform(1600, 900, "Roller game");
	initAudio();
	initRenderer(1600, 900);
	initTextureSystem();
	initFonts();

	platformUpdateLoop(updateGame);
}

void updateGame() {
	if (!game) {
		logf("Game struct is %.2fmb btw\n", (float)sizeof(Game) / (float)Megabytes(1));

		game = (Game *)zalloc(sizeof(Game));
		game->defaultFont = createFont("assets/common/arial.ttf", 20);

		for (int i = 0; i < MAPS_MAX; i++) loadMap(&game->maps[i], i);

		game->timeScale = 1;
		game->nextMap = -1;

		maximizeWindow();
	}

	Globals *globals = &game->globals;

	float elapsed = platform->elapsed * game->timeScale;
	float secondPhase = (sin(game->time*M_PI*2-M_PI*0.5)/2)+0.5;

	game->mouse = (platform->mouse - game->screenOverlayOffset) * (game->size/game->screenOverlaySize);

	{ /// Resizing
		Vec2 ratio = v2(16, 9);
		game->sizeScale = MinNum(platform->windowWidth/ratio.x, platform->windowHeight/ratio.y);
		Vec2 newSize = ratio * game->sizeScale;

		if (!equal(game->size, newSize)) {
			game->size = newSize;

			if (game->gameTexture) destroyTexture(game->gameTexture);
			game->gameTexture = NULL;
			if (game->debugTexture) destroyTexture(game->debugTexture);
			game->debugTexture = NULL;

			game->screenOverlaySize = game->size;
			game->screenOverlayOffset.x = (float)platform->windowWidth/2 - game->size.x/2;
			game->screenOverlayOffset.y = (float)platform->windowHeight/2 - game->size.y/2;
		}
	} ///

	if (!game->gameTexture) game->gameTexture = createRenderTexture(game->size.x, game->size.y);
	if (!game->debugTexture) game->debugTexture = createRenderTexture(game->size.x, game->size.y);

	pushTargetTexture(game->debugTexture);
	clearRenderer();
	popTargetTexture();

	pushTargetTexture(game->gameTexture);
	clearRenderer(0xFF101010);

	if (game->state != game->nextState) {
		game->stateTransition_t += 0.05;
		if (game->stateTransition_t >= 1) {
			game->state = game->nextState;
		}
	} else {
		game->stateTransition_t -= 0.05;
	}
	game->stateTransition_t = Clamp01(game->stateTransition_t);

	if (game->prevState != game->state) {
		game->prevState = game->state;
		game->stateTime = 0;
	}

	if (keyJustPressed(KEY_BACKTICK)) game->inEditor = !game->inEditor;
	// platform->disableGui = !game->inEditor;

	if (game->state == GAME_NONE) {
		if (game->stateTime == 0) game->nextState = GAME_PLAY;
	} else if (game->state == GAME_PLAY) {
		Map *map = &game->maps[game->currentMap];

		float elapsed = platform->elapsed * game->timeScale;
		float secondPhase = (sin(game->time*M_PI*2-M_PI*0.5)/2)+0.5;

		if (game->prevMap != game->currentMap) {
			// Not actually a useful place to put code because you can go to the same map and this won't change
			game->prevMap = game->currentMap;
		}

		Matrix3 cameraMatrix = mat3();
		cameraMatrix.TRANSLATE(-game->cameraTarget + game->size/2);
		pushCamera2d(cameraMatrix);

		game->mouse = cameraMatrix.invert() * game->mouse;

		if (game->inEditor) {
			ImGui::SetNextWindowPos(ImVec2(100, 100), ImGuiCond_Once);
			ImGui::Begin("Editor", NULL, ImGuiWindowFlags_AlwaysAutoResize);
			if (ImGui::TreeNodeEx("Map", ImGuiTreeNodeFlags_DefaultOpen)) {
				ImGui::InputInt("Change map", &game->debugChangeMap);
				game->debugChangeMap = mathClamp(game->debugChangeMap, 0, MAPS_MAX-1);
				if (game->currentMap != game->debugChangeMap) {
					ImGui::SameLine();
					if (ImGui::Button("Go")) {
						game->nextMap = game->debugChangeMap;
					}
				}

				if (ImGui::Button("Save map")) saveMap(map, game->currentMap);
				ImGui::SameLine();
				if (ImGui::Button("Load map")) {
					loadMap(map, game->currentMap);
					game->nextMap = game->currentMap;
				}
				ImGui::InputText("Name", map->name, MAP_NAME_MAX_LEN);

				ImGui::TreePop();
			}

			ImGui::End();

			ImGui::SetNextWindowPos(ImVec2(platform->windowWidth, 0), ImGuiCond_Always, ImVec2(1, 0));
			ImGui::Begin("Actors", NULL, ImGuiWindowFlags_AlwaysAutoResize);
			ImGui::BeginChild("ActorListChild", ImVec2(400, 200));

			for (int i = 0; i < map->actorsNum; i++) {
				ImGui::PushID(i);
				Actor *actor = &map->actors[i];

				ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(0, 0));
				if (ImGui::ArrowButton("upButton", ImGuiDir_Up)) {
					if (i > 0) arraySwap(map->actors, map->actorsNum, sizeof(Actor), i, i-1);
				}
				ImGui::SameLine();
				if (ImGui::ArrowButton("downButton", ImGuiDir_Down)) {
					if (i < map->actorsNum-1) arraySwap(map->actors, map->actorsNum, sizeof(Actor), i, i+1);
				}
				ImGui::PopStyleVar();

				char *name = frameSprintf("%s###%d", actorTypeStrings[actor->type], i);
				ImGui::SameLine();
				if (ImGui::Selectable(name, game->selectedActor == actor->id, 0, ImVec2(320, 0))) {
					game->selectedActor = actor->id;
				}

				ImGui::PopID();
			}
			ImGui::EndChild();
			if (ImGui::Button("Create wall")) {
				Actor *actor = createActor(map, ACTOR_WALL);
				actor->size = v2(128, 128);
				actor->position = game->cameraTarget;
				game->selectedActor = actor->id;
			}

			ImGui::SameLine();
			if (ImGui::Button("Create spawner")) {
				Actor *actor = createActor(map, ACTOR_SPAWNER);
				actor->size = v2(128, 128);
				actor->position = game->cameraTarget;
				game->selectedActor = actor->id;
			}

			ImGui::SameLine();
			if (ImGui::Button("Create door")) {
				Actor *actor = createActor(map, ACTOR_DOOR);
				actor->size = v2(128, 128);
				actor->position = game->cameraTarget;
				game->selectedActor = actor->id;
			}

			ImGui::Separator();

			Actor *actor = getActor(map, game->selectedActor);
			if (actor) {
				if (ImGui::Button("Delete actor")) actor->markedForDeletion = true;
				ImGui::SameLine();
				if (ImGui::Button("Duplicate actor")) {
					Actor *newActor = createActor(map, ACTOR_WALL);
					int id = newActor->id;
					*newActor = *actor;
					newActor->id = id;
					newActor->position += v2(10, 10);
					game->selectedActor = newActor->id;
				}

				ImGui::Combo("Actor type", (int *)&actor->type, actorTypeStrings, ArrayLength(actorTypeStrings));
				ImGui::InputText("Name", actor->name, ACTOR_NAME_MAX_LEN);
				ImGui::SameLine();
				ImGui::Text("Id: %d", actor->id);
				ImGui::DragFloat2("Position", &actor->position.x);
				Vec2 nearest32Pos = roundToNearest(actor->position, v2(32, 32));
				if (!equal(actor->position, nearest32Pos)) {
					ImGui::SameLine();
					if (ImGui::Button("32###32pos")) actor->position = nearest32Pos;
				}
				ImGui::DragFloat2("Size", &actor->size.x, 1);
				Vec2 nearest64Size = roundToNearest(actor->size, v2(64, 64));
				if (!equal(actor->size, nearest64Size)) {
					ImGui::SameLine();
					if (ImGui::Button("64###64size")) actor->size = nearest64Size;
				}

				if (actor->type == ACTOR_SPAWNER) {
					ImGui::Combo("To spawn", (int *)&actor->typeToSpawn, actorTypeStrings, ArrayLength(actorTypeStrings));
				}

				if (actor->type == ACTOR_DOOR) {
					ImGui::InputInt("Door connection id", &actor->doorConnectionId);
				}

				if (actor->type == ACTOR_SPEED_GATE) {
					ImGui::InputFloat("Speed", &actor->speedGateSpeed);
				}

				ImGui::Separator();
			}
			ImGui::End();

			float scrollSpeed = 10;
			if (keyPressed(KEY_LEFT) || keyPressed('A')) game->cameraTarget.x -= scrollSpeed;
			if (keyPressed(KEY_RIGHT) || keyPressed('D')) game->cameraTarget.x += scrollSpeed;
			if (keyPressed(KEY_UP) || keyPressed('W')) game->cameraTarget.y -= scrollSpeed;
			if (keyPressed(KEY_DOWN) || keyPressed('S')) game->cameraTarget.y += scrollSpeed;
		}

		Actor *player = NULL;
		for (int i = 0; i < map->actorsNum; i++) {
			Actor *actor = &map->actors[i];
			if (actor->type == ACTOR_PLAYER) player = actor;
		}

		if (!player) {
			Vec2 spawnPosition = v2();
			for (int i = 0; i < map->actorsNum; i++) {
				Actor *actor = &map->actors[i];
				if (actor->type == ACTOR_DOOR) {
					game->destDoorId = actor->id;
					spawnPosition = actor->position;
					break;
				}
			}

			player = createActor(map, ACTOR_PLAYER);
			player->size = v2(64, 64);
			player->position = spawnPosition;
		}

		/// Actors first iteration
		for (int i = 0; i < map->actorsNum; i++) {
			Actor *actor = &map->actors[i];
		}

		/// Update actors
		for (int i = 0; i < map->actorsNum; i++) {
			Actor *actor = &map->actors[i];

			bool isShell = false;
			bool hasPhysics = false;
			if (actor->type == ACTOR_PLAYER) {
				isShell = true;
				if (!game->inEditor) game->cameraTarget = actor->position;

				if (actor->isOnGround && keyPressed(' ')) actor->velo.y -= 30;

				if (keyPressed('1')) actor->shellSpeed = 5;
				if (keyPressed('2')) actor->shellSpeed = 10;
				if (keyPressed('3')) actor->shellSpeed = 15;
				if (keyPressed('4')) actor->shellSpeed = 20;
			} else if (actor->type == ACTOR_DOOR) {
				bool canUseDoor = true;
				if (game->nextMap == -1 && game->destDoorId == actor->id) {
					canUseDoor = false;
					if (!overlaps(player, actor)) game->destDoorId = -1;
				}
				if (game->nextMap != -1) canUseDoor = false;
				if (canUseDoor && overlaps(player, actor)) {
					int destMapIndex = -1;
					Actor *destDoor = NULL;
					for (int i = 0; i < MAPS_MAX; i++) {
						Map *possibleDestMap = &game->maps[i];
						int possibleDestMapIndex = i;
						for (int i = 0; i < possibleDestMap->actorsNum; i++) {
							Actor *otherMapActor = &possibleDestMap->actors[i];
							if (otherMapActor == actor) continue;
							if (otherMapActor->type == ACTOR_DOOR && otherMapActor->doorConnectionId == actor->doorConnectionId) {
								destMapIndex = possibleDestMapIndex;
								destDoor = otherMapActor;
								break;
							}
						}

						if (destMapIndex != -1) break;
					}

					if (destMapIndex != -1) {
						game->nextMap = destMapIndex;
						game->destDoorId = destDoor->id;
					} else {
						logf("No connecting door to %d\n", actor->doorConnectionId);
					}
				}
			} else if (actor->type == ACTOR_SPEED_GATE) {
				if (overlaps(actor, player)) {
					player->shellSpeed = actor->speedGateSpeed;
				}
			}

			if (isShell) {
				hasPhysics = true;

				if (length(actor->velo) < 0.1) actor->velo = v2(5, -2);

				if (actor->velo.x > 0) {
					actor->velo.x = lerp(actor->velo.x, actor->shellSpeed, 0.1);
				} else if (actor->velo.x < 0) {
					actor->velo.x = lerp(actor->velo.x, -actor->shellSpeed, 0.1);
				} else if (actor->velo.x == 0) {
					actor->velo.x = actor->shellSpeed;
				}
			}

			if (hasPhysics) {
				actor->velo.y += 0.98 * 0.8;

				Vec2 dragFactor = v2(0.96, 0.96);
				if (isShell) dragFactor.x = 1;
				actor->velo *= dragFactor;

				Vec2 newPosition = actor->position + actor->velo;

				actor->isOnGround = false;
				for (int i = 0; i < map->actorsNum; i++) {
					Actor *otherActor = &map->actors[i];
					if (otherActor->type == ACTOR_WALL) {
						Rect wall = getRect(otherActor);
						Rect newRect = getRectFromPosition(actor, newPosition);
						Rect oldRect = getRect(actor);

						if (overlaps(newRect, wall) && newRect.y < wall.y + wall.height && oldRect.y >= wall.y + wall.height) { // Top
							float dist = newRect.y - (wall.y + wall.height);
							newRect.y += dist;
							if (actor->velo.y < 0) actor->velo.y = 0;
						}

						if (overlaps(newRect, wall) && newRect.y + newRect.height > wall.y && oldRect.y + oldRect.height <= wall.y) { // Bot
							float dist = newRect.y + newRect.height - wall.y;
							newRect.y -= dist;
							if (actor->velo.y > 0) actor->velo.y = 0;
							actor->isOnGround = true;
						}

						if (overlaps(newRect, wall) && newRect.x + newRect.width > wall.x && oldRect.x + oldRect.width <= wall.x) { // Right
							float dist = newRect.x + newRect.width - wall.x;
							newRect.x -= dist;
							if (actor->velo.x > 0) {
								if (isShell) {
									actor->velo.x *= -1;
								} else {
									actor->velo.x = 0;
								}
							}
						}

						if (overlaps(newRect, wall) && newRect.x < wall.x + wall.width && oldRect.x >= wall.x + wall.width) { // Left
							float dist = newRect.x - (wall.x + wall.width);
							newRect.x += dist;
							if (actor->velo.x < 0) {
								if (isShell) {
									actor->velo.x *= -1;
								} else {
									actor->velo.x = 0;
								}
							}
						}

						newPosition = getCenter(newRect);
					}
				}

				actor->position = newPosition;
			}
		}

		/// Draw actors
		for (int i = 0; i < map->actorsNum; i++) {
			Actor *actor = &map->actors[i];
			Rect rect = makeRect(actor->position - actor->size/2, actor->size);

			if (actor->type == ACTOR_PLAYER) {
				drawCircle(getCenter(rect), rect.width/2, 0xFF8888FF);
			} else if (actor->type == ACTOR_SPAWNER) {
				if (game->inEditor) drawRect(rect, 0x40FFFFFF);
			} else if (actor->type == ACTOR_WALL) {
				drawRect(rect, 0xFFFFFFFF);
			} else if (actor->type == ACTOR_DOOR) {
				drawRect(rect, 0xFF7D682D);
			} else if (actor->type == ACTOR_SPEED_GATE) {
				drawRect(rect, lerpColor(0x800000FF, 0x80FF0000, clampMap(actor->speedGateSpeed, 5, 50, 0, 1)));
				drawRectOutline(rect, 4, 0xFF000000);
			}

			if (game->selectedActor == actor->id) {
				drawRect(rect, lerpColor(0x40FF00FF, 0x80FF00FF, secondPhase));
			}
		}

		if (game->inEditor) {
			drawCircle(game->mouse, 3, 0xFFFF0000);

			bool selectedSomething = false;

			for (int i = 0; i < map->actorsNum; i++) {
				Actor *actor = &map->actors[i];

				if (platform->mouseJustUp && !game->draggingActor && doesActorContain(actor, game->mouse)) {
					game->selectedActor = actor->id;
					selectedSomething = true;
				}
			}

			if (platform->mouseJustUp && !game->draggingActor && !selectedSomething) {
				game->selectedActor = 0;
			}

			Actor *actor = getActor(map, game->selectedActor);
			if (actor) {
				if (platform->mouseDown && doesActorContain(actor, game->mouse)) {
					if (!game->draggingActor) game->draggingActorOffset = game->mouse - actor->position;
					game->draggingActor = true;
				}

				if (!platform->mouseDown) game->draggingActor = false;

				if (game->draggingActor) {
					if (keyPressed(KEY_SHIFT)) {
						actor->position = roundToNearest(game->mouse - game->draggingActorOffset, v2(32, 32));
					} else {
						actor->position = game->mouse - game->draggingActorOffset;
					}
				}
			}
		}

		/// Delete actors
		for (int i = 0; i < map->actorsNum; i++) {
			Actor *actor = &map->actors[i];
			if (actor->markedForDeletion) {
				deleteActor(map, actor);
			}
		}

		if (game->nextMap != -1) {
			game->mapTransition_t += 0.1;
		} else {
			game->mapTransition_t -= 0.1;
		}
		game->mapTransition_t = Clamp01(game->mapTransition_t);

		if (game->mapTransition_t >= 1) {
			Map *srcMap = &game->maps[game->currentMap];
			Map *destMap = &game->maps[game->nextMap];
			Actor *destDoor = getActor(srcMap, game->destDoorId);
			player->position = destDoor->position;

			if (game->currentMap != game->nextMap) {
				{ // Copy player
					for (;;) {
						Actor *oldPlayer = getActorOfType(destMap, ACTOR_PLAYER);
						if (!oldPlayer) break;
						deleteActor(destMap, oldPlayer);
					}
					Actor *newPlayer = createActor(destMap, ACTOR_PLAYER);
					int id = newPlayer->id;
					*newPlayer = *player;
					newPlayer->id = id;
				}
			}

			game->currentMap = game->nextMap;
			game->nextMap = -1;
		}

		popCamera2d();


		drawRect(makeRect(v2(0, 0), game->size), lerpColor(0x00000000, 0xFF000000, game->mapTransition_t)); // Not far enough
	}

	game->stateTime += elapsed;

	drawRect(makeRect(v2(0, 0), game->size), lerpColor(0x00000000, 0xFF000000, game->stateTransition_t)); // Not far enough
	popTargetTexture(); // game->gameTexture

	clearRenderer();

	{
		Matrix3 matrix = mat3();
		matrix.TRANSLATE(game->screenOverlayOffset);
		matrix.SCALE(game->screenOverlaySize);

		drawSimpleTexture(game->gameTexture, matrix);
		drawSimpleTexture(game->debugTexture, matrix);
	}

	if (keyPressed(KEY_CTRL) && keyPressed(KEY_SHIFT) && keyJustPressed('F')) game->debugShowFrameTimes = !game->debugShowFrameTimes;
	if (game->debugShowFrameTimes) {
		char *str = frameSprintf("%.2fms", platform->frameTimeAvg);
		drawText(game->defaultFont, str, v2(300, 0), 0xFF808080);
	}

	guiDraw();
	drawOnScreenLog();

	game->time += elapsed;
}


Actor *createActor(Map *map, ActorType type) {
	if (map->actorsNum > ACTORS_MAX-1) {
		logf("Too many actors\n");
		map->actorsNum--;
	}

	Actor *actor = &map->actors[map->actorsNum++];
	memset(actor, 0, sizeof(Actor));
	actor->id = ++map->nextActorId;
	actor->type = type;
	actor->shellSpeed = 5;

	return actor;
}

void deleteActor(Map *map, Actor *actor) {
	for (int i = 0; i < map->actorsNum; i++) {
		if (&map->actors[i] == actor) {
			arraySpliceIndex(map->actors, map->actorsNum, sizeof(Actor), i);
			map->actorsNum--;
			i--;
			return;
		}
	}

	logf("Couldn't delete actor, because it's not on this map...\n");
};

Actor *getActor(Map *map, int id) {
	if (id == 0) return NULL;

	for (int i = 0; i < map->actorsNum; i++) {
		Actor *actor = &map->actors[i];
		if (actor->id == id) return actor;
	}
	return NULL;
}

Actor *getActor(Map *map, char *name) {
	if (!name || !name[0]) return NULL;

	for (int i = 0; i < map->actorsNum; i++) {
		Actor *actor = &map->actors[i];
		if (streq(actor->name, name)) return actor;
	} 

	return NULL;
}

Actor *getActorOfType(Map *map, ActorType type) {
	for (int i = 0; i < map->actorsNum; i++) {
		Actor *actor = &map->actors[i];
		if (actor->type == type) return actor;
	} 

	return NULL;
}

Actor *getActorWithDoorConnectionId(Map *map, int doorConnectionId) {
	for (int i = 0; i < map->actorsNum; i++) {
		Actor *actor = &map->actors[i];
		if (actor->doorConnectionId == doorConnectionId) return actor;
	} 

	return NULL;
}

bool doesActorContain(Actor *actor, Vec2 point) {
	Rect rect = getRect(actor);
	return contains(rect, point);
}

bool overlaps(Actor *actorA, Actor *actorB) {
	return overlaps(getRect(actorA), getRect(actorB));
}

void saveMap(Map *map, int mapFileIndex) {
	DataStream *stream = newDataStream();

	int MAP_VERSION = 4;
	writeU32(stream, MAP_VERSION);

	writeString(stream, map->name);

	int actorsToSave = 0;
	for (int i = 0; i < map->actorsNum; i++) {
		Actor *actor = &map->actors[i];
		bool willSave = true;
		if (actor->type == ACTOR_PLAYER) willSave = false;
		if (willSave) actorsToSave++;
	}

	writeU32(stream, actorsToSave);
	for (int i = 0; i < map->actorsNum; i++) {
		Actor *actor = &map->actors[i];
		bool willSave = true;
		if (actor->type == ACTOR_PLAYER) willSave = false;
		if (!willSave) continue;

		writeU32(stream, actor->type);
		writeString(stream, actor->name);
		writeU32(stream, actor->id);
		writeVec2(stream, actor->position);
		writeVec2(stream, actor->size);
		writeU32(stream, actor->typeToSpawn);
		writeU32(stream, actor->flags);
		writeU32(stream, actor->doorConnectionId);
		writeFloat(stream, actor->speedGateSpeed);
	}
	writeU32(stream, map->nextActorId);

	writeDataStream(frameSprintf("assets/maps/map%d.bin", mapFileIndex), stream);
	destroyDataStream(stream);
}

void loadMap(Map *map, int mapFileIndex) {
	memset(map, 0, sizeof(Map));

	DataStream *stream = loadDataStream(frameSprintf("assets/maps/map%d.bin", mapFileIndex));
	if (!stream) {
		// logf("No map at index %d\n", mapFileIndex);
		return;
	}

	int version = readU32(stream);

	readStringInto(stream, map->name, MAP_NAME_MAX_LEN);

	map->actorsNum = readU32(stream);
	memset(map->actors, 0, sizeof(Actor) * ACTORS_MAX);
	for (int i = 0; i < map->actorsNum; i++) {
		Actor *actor = &map->actors[i];
		actor->type = (ActorType)readU32(stream);
		readStringInto(stream, actor->name, ACTOR_NAME_MAX_LEN);
		actor->id = readU32(stream);
		actor->position = readVec2(stream);
		actor->size = readVec2(stream);
		actor->typeToSpawn = (ActorType)readU32(stream);
		if (version >= 2) actor->flags = readU32(stream);
		if (version >= 3) actor->doorConnectionId = readU32(stream);
		if (version >= 4) actor->speedGateSpeed = readFloat(stream);
	}

	map->nextActorId = readU32(stream);

	destroyDataStream(stream);
}
