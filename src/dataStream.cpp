...
SimpleXform2 readSimpleXform2(DataStream *stream);
...
void writeXform2(DataStream *stream, SimpleXform2 value) { writeBytes(stream, &value, sizeof(SimpleXform2)); }
...
SimpleXform2 readSimpleXform2(DataStream *stream) {
	SimpleXform2 ret;
	readBytes(stream, &ret, sizeof(SimpleXform2));
	return ret;
}
